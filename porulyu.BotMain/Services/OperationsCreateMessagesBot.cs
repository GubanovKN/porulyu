...
namespace porulyu.BotMain.Services
{
    public class OperationsCreateMessagesBot
    {
        ...
        public InlineKeyboardButton[][] GetMarksButtons(User user)
        {
            double Pages = Constants.Marks.Count / 12.0;

            int countKeyBoard = (Constants.Marks.Count - user.PositionView * 12) / 2;

            InlineKeyboardButton[][] inlineKeyboards;

            int row = 0;

            inlineKeyboards = new InlineKeyboardButton[countKeyBoard > 6 ? 7 : (Constants.Marks.Count - user.PositionView * 12) % 2 == 0 ? countKeyBoard + 1 : countKeyBoard + 2][];

            for (int i = 0; user.PositionView * 12 + i < Constants.Marks.Count && i < 12; i = i + 2)
            {
                if (user.PositionView * 12 + i == Constants.Marks.Count - 1)
                {
                    inlineKeyboards[row] = new InlineKeyboardButton[]
                    {
                       InlineKeyboardButton.WithCallbackData(Constants.Marks[user.PositionView * 12 + i].Name, Constants.Marks[user.PositionView * 12 + i].Alias)
                    };
                }
                else
                {
                    ...
                    {
                       InlineKeyboardButton.WithCallbackData(Constants.Marks[user.PositionView * 12 + i].Name, Constants.Marks[user.PositionView * 12 + i].Alias),
                       InlineKeyboardButton.WithCallbackData(Constants.Marks[user.PositionView * 12 + i + 1].Name, Constants.Marks[user.PositionView * 12 + i + 1].Alias),
                    };
                }
        ...
        public InlineKeyboardButton[][] GetModelsButtons(User user)
        {
            Mark Mark = Constants.Marks.Find(p => p.Alias == user.Filters.Last().MarkAlias);

            double Pages = Mark.Models.Count / 12.0;

            int countKeyBoard = (Mark.Models.Count - user.PositionView * 12) / 2;

            InlineKeyboardButton[][] inlineKeyboards;

            int row = 0;

            inlineKeyboards = new InlineKeyboardButton[countKeyBoard > 6 ? 7 : (Mark.Models.Count - user.PositionView * 12) % 2 == 0 ? countKeyBoard + 1 : countKeyBoard + 2][];

            for (int i = 0; user.PositionView * 12 + i < Mark.Models.Count && i < 12; i = i + 2)
            {
                if (user.PositionView * 12 + i == Mark.Models.Count - 1)
                {
                    inlineKeyboards[row] = new InlineKeyboardButton[]
                    {
                       InlineKeyboardButton.WithCallbackData(Mark.Models[user.PositionView * 12 + i].Name, Mark.Models[user.PositionView * 12 + i].Alias)
                    };
                }
                else
                {
                    ...
                    {
                       InlineKeyboardButton.WithCallbackData(Mark.Models[user.PositionView * 12 + i].Name, Mark.Models[user.PositionView * 12 + i].Alias),
                       InlineKeyboardButton.WithCallbackData(Mark.Models[user.PositionView * 12 + i + 1].Name, Mark.Models[user.PositionView * 12 + i + 1].Alias),
                    };
                }
        ...
        public InlineKeyboardButton[][] GetRegionsButtons()
        {
            InlineKeyboardButton[][] inlineKeyboards = new InlineKeyboardButton[8][];

            int row = 0;

            for (int i = 0; i < Constants.Regions.Count; i = i + 2)
            {
                inlineKeyboards[row] = new InlineKeyboardButton[]
                {
                       InlineKeyboardButton.WithCallbackData(Constants.Regions[i].Name, Constants.Regions[i].Alias),
                       InlineKeyboardButton.WithCallbackData(Constants.Regions[i+1].Name, Constants.Regions[i+1].Alias),
                };
        ...
        public InlineKeyboardButton[][] GetCitiesButtons(User user)
        {
            var Region = Constants.Regions.Find(p => p.Alias == user.Filters.Last().RegionAlias);

            double Pages = Region.Cities.Count / 12.0;

            int countKeyBoard = (Region.Cities.Count - user.PositionView * 12) / 2;

            InlineKeyboardButton[][] inlineKeyboards;

            int row = 0;

            inlineKeyboards = new InlineKeyboardButton[countKeyBoard > 6 ? 7 : (Region.Cities.Count - user.PositionView * 12) % 2 == 0 ? countKeyBoard + 1 : countKeyBoard + 2][];

            for (int i = 0; user.PositionView * 12 + i < Region.Cities.Count && i < 12; i = i + 2)
            {
                if (user.PositionView * 12 + i == Region.Cities.Count - 1)
                {
                    inlineKeyboards[row] = new InlineKeyboardButton[]
                    {
                       InlineKeyboardButton.WithCallbackData(Region.Cities[user.PositionView * 12 + i].Name, Region.Cities[user.PositionView * 12 + i].Alias)
                    };
                }
                else
                {
                    ...
                    {
                       InlineKeyboardButton.WithCallbackData(Region.Cities[user.PositionView * 12 + i].Name, Region.Cities[user.PositionView * 12 + i].Alias),
                       InlineKeyboardButton.WithCallbackData(Region.Cities[user.PositionView * 12 + i + 1].Name, Region.Cities[user.PositionView * 12 + i + 1].Alias),
                    };
                }
        ...
        {
            string Text = $"Введите пробег автомобиля в формате (1-10000000) км.\n\r" +
                          $"Если не хотите указывать начальный или конечный пробег, просто оставтье его место пустым.";
        ...
        public string GetSaveText(Filter filter)
        {
            string Params = $"<b>Параметры фильтра</b>\n\r";

            Params += $"Имя фильтра: {filter.Name}\n\r";

            if (filter.MarkAlias != null)
            {
                Mark mark = Constants.Marks.FirstOrDefault(p => p.Alias == filter.MarkAlias);

                Params += $"Марка: {mark.Name}\n\r";

                if (filter.ModelAlias != null)
                {
                    Model model = mark.Models.FirstOrDefault(p => p.Alias == filter.ModelAlias);
            ...
            if (filter.RegionAlias != null)
            {
                Region region = Constants.Regions.FirstOrDefault(p => p.Alias == filter.RegionAlias);

                Params += $"Регион: {region.Name}\n\r";

                if (filter.CityAlias != null)
                {
                    City city = region.Cities.FirstOrDefault(p => p.Alias == filter.CityAlias);
            ...
            if (filter.FirstMileage != 0 && filter.SecondMileage != 0)
            {
                Params += $"Пробег: от {filter.FirstMileage} до {filter.SecondMileage}\n\r";
            }
            else if (filter.FirstMileage != 0)
            {
                Params += $"Пробег: от {filter.FirstMileage}\n\r";
            }
            else if (filter.SecondMileage != 0)
            {
                Params += $"Пробег: до {filter.SecondMileage}\n\r";
            }
            else
            {
        ...
        public InlineKeyboardButton[][] GetRatesButtons()
        {
            InlineKeyboardButton[][] inlineKeyboards = new InlineKeyboardButton[2][];
